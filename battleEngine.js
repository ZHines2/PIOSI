...
export class BattleEngine {
  constructor(party, enemies, fieldRows, fieldCols, wallHP, onLevelComplete, onGameOver) {
    ...
    this.getLiveHeroes().forEach(hero => {
      if (hero.caprice && hero.caprice > 0) {
        ...
        for (let i = 0; i < hero.caprice; i++) {
          ...
          hero[randomStat] += 1;
          logMessage(`${hero.name}'s caprice boosts ${randomStat} to ${hero[randomStat]}`);
        }
      }
    });
    this.getLiveHeroes().forEach(hero => {
      if (hero.fate && hero.fate > 0) {
        ...
        for (let i = 0; i < hero.fate; i++) {
          ...
          logMessage(`${hero.name}'s fate changes ${randomFate.stat to ${hero[randomFate.stat]}`);
        }
      }
    });
    return field;
  ...
  moveUnit(dx, dy) {
    ...
    if (unit.hp <= 0) {
      logMessage(`${unit.name} is dead and cannot move.`);
      return;
    }
    ...
    if (this.battlefield[newY][newX] === 'ᚙ' || this.battlefield[newY][newX] === '█') {
      ...
      logMessage(`${unit.name} attacks the wall for ${unit.attack} damage! (Wall HP: ${this.wallHP})`);
    ...
    if (this.battlefield[newY][newX] === 'ౚ') {
      ...
      logMessage(`${unit.name} picks up a vittle and heals for ${healingValue} HP! (New HP: ${unit.hp})`);
      this.battlefield[newY][newX] = '.';
    }
    if (this.battlefield[newY][newX] === 'ඉ') {
      const healingValue = 5;
      unit.hp += healingValue;
      logMessage(`${unit.name} picks up a mushroom and heals for ${healingValue} HP! (New HP: ${unit.hp})`);
      this.battlefield[newY][newX] = '.';
      if (unit.spore && unit.spore > 0) {
        const stats = ['attack', 'range', 'agility', 'hp'];
        const randomStat = stats[Math.floor(Math.random() * stats.length)];
        unit[randomStat] += unit.spore;
        logMessage(`${unit.name} gains ${unit.spore} boost to ${randomStat} (Now: ${unit[randomStat]})`);
      }
    }
  ...
  async attackInDirection(dx, dy, unit, recordAttackCallback) {
    ...
    if (unit.hp <= 0) {
      logMessage(`${unit.name} is dead and cannot attack.`);
      return;
    }
    ...
    for (let i = 1; i <= unit.range; i++) {
      ...
      if (ally) {
        if (unit.heal && unit.heal > 0) {
          ally.hp += unit.heal;
          logMessage(`${unit.name} heals ${ally.name} for ${unit.heal} HP! (New HP: ${ally.hp})`);
        } else if (unit.psych && unit.psych > 0) {
          const stats = ['attack', 'range', 'agility', 'hp'];
          const randomStat = stats[Math.floor(Math.random() * stats.length)];
          ally[randomStat] += unit.psych;
          logMessage(`${unit.name} uses psych on ${ally.name}, boosting ${randomStat} by ${unit.psych}! (New ${randomStat}: ${ally[randomStat]})`);
        } else {
          logMessage(`${unit.name} attacks ${ally.name} but nothing happens.`);
        }
        this.awaitingAttackDirection = false;
      ...
      if (deadHero) {
        logMessage(`${unit.name} attacks an empty cell where ${deadHero.name} once stood.`);
        this.awaitingAttackDirection = false;
        await this.shortPause();
        this.nextTurn();
        return;
      ...
      if (enemy) {
        ...
        if (Math.random() < dodgeChance) {
          logMessage(`${enemy.name} dodges ${unit.name}'s attack!`);
          this.awaitingAttackDirection = false;
          await this.shortPause();
          this.nextTurn();
        ...
        if (unit.trick > 0) {
          ...
          if (availableStats.length > 0) {
            ...
            logMessage(`${unit.name}'s trick lowers ${enemy.name}'s ${chosenStat} from ${orig} to ${enemy[chosenStat]}!`);
          }
        }
        if (unit.burn) {
          enemy.statusEffects.burn = { damage: unit.burn, duration: 3 };
          logMessage(`${enemy.name} is burning for ${unit.burn} damage for 3 turns!`);
        }
        if (unit.sluj) {
          ...
          logMessage(`${enemy.name} is afflicted with slüj (level ${enemy.statusEffects.sluj.level}) for 4 turns!`);
        }
        if (unit.yeet && unit.yeet > 0) {
          applyKnockback(enemy, dx, dy, unit.yeet, unit.attack, this.battlefield, logMessage, this.isWithinBounds.bind(this));
        }
        if (unit.chain) {
          ...
          if (initialChainDamage > 0) {
            logMessage(`${enemy.name} takes ${initialChainDamage} chain damage!`);
        ...
        adjacentOffsets.forEach(offset => {
          ...
          if (adjacentHero) {
            ...
            logMessage(`${adjacentHero.name}'s bomba deals ${adjacentHero.bomba} additional damage to ${enemy.name}! (HP left: ${enemy.hp})`);
          }
        });
    ...
    logMessage(`${unit.name} attacks, but nothing is in range.`);
    this.awaitingAttackDirection = false;
    await this.shortPause();
    this.nextTurn();
  }
  ...
  applyChainDamage(enemy, damage, effectiveMultiplier, visited = new Set()) {
    ...
    for (let offset of adjacentOffsets) {
      ...
      if (adjacentEnemy && !visited.has(adjacentEnemy)) {
        ...
        logMessage(`${adjacentEnemy.name} takes ${damage} chain damage! (HP left: ${adjacentEnemy.hp})`);
        if (adjacentEnemy.hp <= 0) {
          logMessage(`${adjacentEnemy.name} is defeated by chain damage!`);
        ...
        if (nextDamage > 0 && nextDamage < damage) {
          logMessage(`${adjacentEnemy.name} takes ${nextDamage} chain propagation damage!`);
  ...
  enemyTurn() {
    ...
    this.enemies.forEach(enemy => {
      ...
      if (Array.isArray(enemy.dialogue) && enemy.dialogue.length > 0) {
        logMessage(`${enemy.name} says: "${enemy.dialogue[Math.floor(Math.random() * enemy.dialogue.length)]}"`);
      }
    });
  ...
  enemyAttackAdjacent(enemy) {
    ...
    directions.forEach(([dx, dy]) => {
      ...
      if (targetHero) {
        ...
        if (Math.random() < dodgeChance) {
          logMessage(`${targetHero.name} dodges ${enemy.name}'s attack!`);
          return;
        }
        // DODGE CHECK END
        if (targetHero.armor && targetHero.armor > 0) {
          targetHero.armor--;
          logMessage(`${enemy.name} attacks ${targetHero.name} but their armor absorbs it (Remaining Armor: ${targetHero.armor})`);
        } else {
          targetHero.hp -= enemy.attack;
          logMessage(`${enemy.name} attacks ${targetHero.name} for ${enemy.attack} damage! (HP left: ${targetHero.hp})`);
        }
        if (targetHero.hp <= 0) {
          ...
          if (targetHero.hasOwnProperty(randomStat)) {
            ...
            logMessage(`${targetHero.name}'s rage boosts ${randomStat} by ${targetHero.rage} (Now: ${targetHero[randomStat]})`);
          }
        }
      }
    });
  ...
  nextTurn() {
    ...
    if (liveHeroes.length === 0) {
      logMessage('All heroes defeated! Game Over.');
    ...
    logMessage(`Now it's ${this.party[this.currentUnit].name}'s turn.`);
  }

  applyStatusEffects() {
    this.getLiveHeroes().forEach(hero => {
      if (hero.statusEffects.burn && hero.statusEffects.burn.duration > 0) {
        logMessage(`${hero.name} takes ${hero.statusEffects.burn.damage} burn damage!`);
    ...
    this.enemies.forEach(enemy => {
      if (enemy.statusEffects.burn && enemy.statusEffects.burn.duration > 0) {
        logMessage(`${enemy.name} takes ${enemy.statusEffects.burn.damage} burn damage!`);
        enemy.hp -= enemy.statusEffects.burn.damage;
        enemy.statusEffects.burn.duration--;
        if (enemy.hp <= 0) {
          logMessage(`${enemy.name} died from burn damage!`);
          this.battlefield[enemy.y][enemy.x] = '.';
          this.enemies = this.enemies.filter(e => e !== enemy);
        }
      }
      ...
      if (hero.swarm && typeof hero.swarm === 'number') {
        adjacentOffsets.forEach(offset => {
          ...
          if (this.isWithinBounds(targetX, targetY)) {
            ...
            if (enemy) {
              enemy.hp -= hero.swarm;
              logMessage(`${hero.name}'s swarm deals ${hero.swarm} damage to ${enemy.name} at (${targetX},${targetY}) (HP left: ${enemy.hp})`);
              if (enemy.hp <= 0) {
                logMessage(`${enemy.name} is defeated by swarm damage!`);
  ...
  handleHeroDeath(hero) {
    if (hero.rise > 0) {
      logMessage(`Hero ${hero.name} falls but rises with ${hero.rise} HP!`);
      hero.hp = hero.rise;
      hero.rise = 0;
      this.applyAnkhBoost();
      return;
    ...
    if (hero.persistentDeath) return;
    logMessage(`Hero ${hero.name} has fallen permanently. Applying persistent death and ankh effects...`);
  ...
  applyAnkhBoost() {
    this.getLiveHeroes().forEach(h => {
      if (h.ankh && typeof h.ankh === 'number' && h.ankh > 0) {
        ...
        const randomStat = stats[Math.floor(Math.random() * stats.length)];
        h[randomStat] += h.ankh;
        logMessage(`${h.name} gains an ankh boost of ${h.ankh} ${randomStat} (Now: ${h[randomStat]}).`);
      }
    });
  }
